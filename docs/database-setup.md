...
A web-based database management GUI is available at:

```
http://10.65.65.15:8080/
```

Login fields:
- **Server**: `db` (Docker Compose service name — do NOT change this)
- **Username**: value from `$DB_APP_USER`
- **Password**: value from `$DB_APP_PASSWORD`
- **Database**: your app's database name (e.g. `thc_portal`)

Use this for browsing tables, running queries, inspecting schema, and managing data without SSH.
